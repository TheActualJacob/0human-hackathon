...
import {
  ...
  Map as MapIcon, List, Car, PawPrint, X
} from 'lucide-react';
...
export default function PropertiesPage() {
  ...
  const [geocodedPins, setGeocodedPins] = useState<globalThis.Map<string, GeocodedPin>>(() => new globalThis.Map());
  ...
  // Geocode all units once when data loads — pins are cached for the session
  useEffect(() => {
    if (allUnits.length === 0 || geocodingRef.current) return;
    geocodingRef.current = true;
    setGeocoding(true);

    let remaining = allUnits.length;
    let cancelled = false;

    allUnits.forEach(async (unit) => {
      try {
        ...
        if (coords && !cancelled) {
          setGeocodedPins(prev => {
            ...
            next.set(unit.id, { id: unit.id, lat: coords.lat, lng: coords.lng, unit });
            return next;
          });
        }
      } finally {
  ...
  const mapPins = useMemo<GeocodedPin[]>(
    () => units.map(u => geocodedPins.get(u.id)).filter((p): p is GeocodedPin => !!p),
    [units, geocodedPins]
  );
  ...
  return (
    <div className="min-h-screen bg-[#05050a] text-white">
      ...
      {/* Navbar — only for unauthenticated visitors */}
      {!isLoggedIn && (
      ...
      {/* Hero — only for unauthenticated visitors */}
      {!isLoggedIn && <div className="relative overflow-hidden border-b border-white/5">
      ...
      {/* Authenticated: compact inline search bar */}
      {isLoggedIn && (
        <div className="px-6 pt-6 pb-2">
          <div className="flex items-center gap-2">
            <div className="flex-1 flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2 focus-within:border-indigo-500/50 transition-colors">
              <Search className="h-4 w-4 text-white/30 flex-shrink-0" />
              <input
                className="flex-1 bg-transparent text-white placeholder-white/30 outline-none text-sm"
                placeholder="Search by address, city, or description…"
